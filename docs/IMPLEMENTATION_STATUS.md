...
- The wordmark is 25% smaller at every viewport width; How the club works has a separating horizontal rule. Outbound, artwork-credit and privacy links open in a new tab with noopener. Home and section links keep visitors in the current tab.
- The opening uses less space above/below the wordmark and menu, a wider headline measure and tighter introductory margins. This brings signup into the initial laptop viewport while preserving the selected font/logo sizes, readable copy, button targets and later section spacing.
- Section navigation starts with Home to the root route, then uses “October’s Book” for its book link and links to labelled visible blocks. Links follow page order, exclude hidden/removed sections, and support editable labels. In the editor, Home scrolls to the masthead without reloading the unsaved canvas. Native anchors need no public script; the larger-screen sticky bar has scroll offsets, keyboard focus targets and reduced-motion support. Mobile navigation wraps in normal flow. Menu jumps do not outline or select the destination; explicit editor selection and interactive keyboard focus indicators remain available.
- Lower-page sections use full-width rows with headings beside their text on desktop, bounded text line lengths, and full-width FAQ rules. The editor exposes this as “Heading beside text”; smaller screens stack the content. Opening, image and prose sections use a consistent 900px transition; zero-minimum grid tracks, long-text wrapping and adaptable dates protect narrower views and enlarged text.
...
- Targeted browser checks on 13 September confirmed that the current split-layout waitlist button fits without scrolling at 1280×600, 1280×720, 1440×800, 768×1024 and 390×844. None of those widths had horizontal page overflow. The phone opening was visually inspected; the editor's saved preview was refreshed without changing its draft revision.
